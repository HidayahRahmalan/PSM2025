...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($email)) {
        try {
            $stmt = $conn->prepare("SELECT StaffEmail FROM EMPLOYEE WHERE StaffEmail = ?");
    ...
    if (!empty($personalEmail)) {
        try {
            $stmt = $conn->prepare("SELECT PersonalEmail FROM EMPLOYEE WHERE PersonalEmail = ?");
    ...
    if (!empty($phone)) {
        try {
            $stmt = $conn->prepare("SELECT PhoneNo FROM EMPLOYEE WHERE PhoneNo = ?");
