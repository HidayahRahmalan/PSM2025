<?php
// This file is included by admViewAuditTrail.php to fetch audit trails based on search criteria

// Initialize the audit trails array
$auditTrails = [];
...
$allowedSortColumns = ['TrailID','RecordID', 'UserID', 'FullName', 'TimeStamp'];
...
try {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $row['TableName'] = ucwords(strtolower($row['TableName']));
        
        // Add to the audit trails array
        $auditTrails[] = $row;
        $totalResults++;
    }
    ...
    if ($sortBy === 'FullName') {
        usort($auditTrails, function($a, $b) use ($sortOrder) {
