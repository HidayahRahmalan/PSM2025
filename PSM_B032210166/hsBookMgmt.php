...
$hostelStaffName = "";
try {
    $stmt = $conn->prepare("SELECT FullName FROM EMPLOYEE WHERE EmpID = ? AND Role = 'HOSTEL STAFF'");
...
<head>
    ...
    <title>Booking Management - Hostel Staff</title>
...
<body>
    ...
    <script>
        ...
        function editBooking(bookID) {
            ...
            fetch('hsBookingActions.php?action=getBooking&bookID=' + encodeURIComponent(bookID))
                .then(response => {
                    if (!response.ok) {
        ...
        function viewRoomDetails() {
            ...
            // Fetch room details
            fetch('hsBookingActions.php?action=getRoomDetails&roomNo=' + encodeURIComponent(roomNo) + '&hostelName=' + encodeURIComponent(hostelName))
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
                    }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (status === 'APPROVED' || status === 'REJECTED') {
                        ...
                        return fetch('hsSendBookingStatusEmail.php', {
                            method: 'POST',
                            body: emailData
                        })
