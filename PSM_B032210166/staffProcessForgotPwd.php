...
function checkEmailExists() {
    ...
    try {
        // Check if email exists in EMPLOYEE table and get personal email
        $stmt = $conn->prepare("SELECT EmpID, StaffEmail, PersonalEmail, Status FROM EMPLOYEE WHERE StaffEmail = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            if (empty($employee['PersonalEmail'])) {
...
function resetPassword() {
    ...
    try {
        ...
        $stmt = $conn->prepare("UPDATE EMPLOYEE SET Password = ? WHERE StaffEmail = ?");
