...
// Fetch user's saved recipes with totalcalorie
...
foreach ($recipes as $recipe) {
    $recipeCalories[$recipe['recipeid']] = $recipe['totalcalories'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($title) && !empty($selectedRecipes)) {
        // Insert into mealplan with totalmealcalorie
        $insertMeal = $conn->prepare("INSERT INTO mealplan (id, title, description, totalmealcalorie) VALUES (?, ?, ?, ?)");
        $insertMeal->bind_param("issd", $id, $title, $description, $totalMealCalorie);
        $insertMeal->execute();
        ...
        // Insert selected recipes into mealplanrecipe
...
<head>
    ...
    <title>New Meal Plan - NutriEats</title>
    <style>
        * {
...
<body>
    ...
    <div class="container">
        ...
        <form method="POST" action="NewMealPlan.php" id="mealPlanForm">
            ...
            <div id="recipeContainer">
                <div class="recipe-group">
                    <select name="recipes[]" onchange="updateTotalCalorie()" required>
                        <option value="">-- Select Recipe --</option>
                        <?php foreach ($recipes as $recipe): ?>
                            <option value="<?php echo $recipe['recipeid']; ?>" data-calorie="<?php echo $recipe['totalcalories']; ?>">
            ...
            <h3>Total Meal Calorie: <span id="totalCalorieDisplay">0</span> kcal</h3>
            <input type="hidden" name="total_meal_calorie" id="totalMealCalorie" value="0">
    ...
    <script>
        const recipeCalories = <?php echo json_encode($recipeCalories); ?>;
    ...
    // Dynamically add options using the JavaScript array
